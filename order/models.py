...
class OrderItem(models.Model):
    ...
    quantity = models.PositiveIntegerField(default=1, validators=[MinValueValidator(1)])  # Ensures a valid, non-zero quantity
    date_added = models.DateTimeField(auto_now_add=True) 

    def __str__(self):
        return f"{self.quantity} of {self.product.name if self.product else 'Unknown Product'} in Order for {self.order.user}"  # Displays item details
